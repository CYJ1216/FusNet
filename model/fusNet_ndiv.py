...
class FusNet_ndiv(nn.Module):
    # res2net based encoder decoder
    def __init__(self):
        super(FusNet_ndiv, self).__init__()
        # ---- ResNet Backbone ----
    ...
    def forward(self, x):
        ...
        x5_dem_5 = self.x5_dem_5(x5_4)
        output4 = self.output4(F.upsample(x5_dem_1,size=x4_dem_1.size()[2:], mode='bilinear') + x4_dem_1)
        output3 = self.output3(F.upsample(output4,size=x3_dem_1.size()[2:], mode='bilinear') + x3_dem_1)
        output2 = self.output2(F.upsample(output3,size=x2_dem_1.size()[2:], mode='bilinear') + x2_dem_1)
        output1 = self.output1(F.upsample(output2,size=x2_1.size()[2:], mode='bilinear') + x2_1)
...
if __name__ == '__main__':
    ras = FusNet_ndiv(n_classes=20).cuda()
