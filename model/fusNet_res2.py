...
class FusNet_res2(nn.Module):
    # res2net based encoder decoder
    def __init__(self):
        super(FusNet_res2, self).__init__()
        # ---- ResNet Backbone ----
        ...
        self.x5_dem_1 = nn.Sequential(nn.Conv2d(2048, 96, kernel_size=3, padding=1), nn.BatchNorm2d(96),
                                      nn.ReLU(inplace=True))
        self.x4_dem_1 = nn.Sequential(nn.Conv2d(1024, 96, kernel_size=3, padding=1), nn.BatchNorm2d(96),
                                      nn.ReLU(inplace=True))
        self.x3_dem_1 = nn.Sequential(nn.Conv2d(512, 96, kernel_size=3, padding=1), nn.BatchNorm2d(96),
                                      nn.ReLU(inplace=True))
        self.x2_dem_1 = nn.Sequential(nn.Conv2d(256, 96, kernel_size=3, padding=1), nn.BatchNorm2d(96),
                                      nn.ReLU(inplace=True))  # 第一层加强层
    ...
    def forward(self, x):
        ...
        #进入加强层
        x5_dem_1 = self.x5_dem_1(x5r)   #MS模块第一层
        x4_dem_1 = self.x4_dem_1(x4r)   #MS模块第一层
        x3_dem_1 = self.x3_dem_1(x3r)   #MS模块第一层
        x2_dem_1 = self.x2_dem_1(x2r)   #MS模块第一层
        #第一层减法单元
...
if __name__ == '__main__':
    ras = FusNet_res2(n_classes=20).cuda()
