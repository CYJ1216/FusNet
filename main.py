...
def getArgs():
    ...
    parse.add_argument('--arch', '-a', metavar='ARCH', default='FusNet',
    ...
    parse.add_argument('--dataset', default='KvasirSEG',  # dsb2018_256
...
def train(model, criterion, optimizer, train_dataloader,val_dataloader, args):
    ...
    for epoch in range(num_epochs):
        ...
        best_iou_ ,aver_iou_ ,aver_acc_, aver_dice_ = 0, 0, 0, 0
        for i in range(5):
            epoch_loss = 0
            train_dataloader.dataset.dataset_num = i
            val_dataloader.dataset.dataset_num = i
            for x, y,_,mask in train_dataloader:
                step += 1
                ...
                if threshold!=None:
                    ...
                    optimizer.step()
                    epoch_loss += loss.item()
                cross_loss+= epoch_loss/5


                print("%d %d %d/%d,train_loss:%0.3f" % (epoch,i,step, (dt_size - 1) // train_dataloader.batch_size + 1, loss.item()))
                logging.info("%d/%d,train_loss:%0.3f" % (step, (dt_size - 1) // train_dataloader.batch_size + 1, loss.item()))
            cross_loss += epoch_loss/5

            best_iou,aver_iou,aver_acc,aver_dice = val(model,best_iou,val_dataloader)
            best_iou_ += best_iou/5
            aver_iou_ += aver_iou/5
            aver_acc_ += aver_acc/5
            aver_dice_ += aver_dice/5
        loss_list.append(cross_loss)
        iou_list.append(aver_iou_)
        acc_list.append(aver_acc_)
        dice_list.append(aver_dice_)
        print("epoch %d loss:%0.3f" % (epoch, cross_loss))
        logging.info("epoch %d loss:%0.3f best_iou:%0.3f" % (epoch, cross_loss,best_iou_))
    loss_plot(args, loss_list)
    metrics_plot(args, 'iou&acc',iou_list, acc_list)
    metrics_plot(args,'hd',dice_list)
    return model
